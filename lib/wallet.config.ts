...
// --- INTERFACES ---
export interface Transaction { id: string; nombre: string; monto: number; cuil: string; hora: string | null; }
export interface FetchTransactionsResponse { transactions: Transaction[]; totalPages: number; }
export interface FetchOutflowsResponse { outflows: Transaction[]; totalAmount: number; }
...
const getMercadoPagoIncoming = (accessToken: string, userId: string) => async (date: string, page: number): Promise<FetchTransactionsResponse> => {
    ...
    // Convertimos el ID de texto a número para una comparación correcta
    const myUserId = parseInt(userId, 10);

    const filtered = data.results.filter((p: any) =>
      p.status === 'approved' &&
      p.collector_id === myUserId && // Ahora sí, comparamos número con número
...
const getMercadoPagoOutflows = (accessToken: string, userId: string) => async (date: string): Promise<FetchOutflowsResponse> => {
    ...
    const outflows = data.results
      .filter((p: any) => {
        // Unificamos la lógica para usar siempre el comparador estricto (===)
